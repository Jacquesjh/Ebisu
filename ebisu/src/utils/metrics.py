...
def get_metrics(qs_metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    metrics.loc["Longest DD Days"] = metrics.loc["Longest DD Days"][0] / pow(
        10, mdd_algarism
    )

    mdd_algarism = len(str(metrics.loc["Avg. Drawdown Days"][0]))
    metrics.loc["Avg. Drawdown Days"] = metrics.loc["Avg. Drawdown Days"][0] / pow(
        10, mdd_algarism
    )

    metrics.loc["Expected Shortfall (cVaR)"] = metrics.loc["Expected Shortfall (cVaR)"][
        0
    ]
    metrics.loc["Max Drawdown"] = metrics.loc["Max Drawdown"][0]
    metrics.loc["Daily Value-at-Risk"] = metrics.loc["Daily Value-at-Risk"][0]
    metrics.loc["RoMad"] = (
        qs_metrics.loc["Cumulative Return"] / qs_metrics.loc["Max Drawdown"]
    )[0]
