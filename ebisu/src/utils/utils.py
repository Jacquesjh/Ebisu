...
def split_list(my_list: list, n_parts: int) -> Callable:
    ...
    return (
        my_list[i * k + min(i, m) : (i + 1) * k + min(i + 1, m)] for i in range(n_parts)
    )


def get_dates(
    raw_df: pd.DataFrame, rebalance_interval: str, start_date: datetime
) -> List[datetime]:
    table = {
...
def validate_rebalance_intervals(interval: str) -> None:
    ...
    if interval not in valid_rebalance_intervals:
        raise ValueError(
            f"The given rebalance interval {interval} is not valid. Must be one of {valid_rebalance_intervals}."
        )
...
def validate_domain_sectors(sectors: List[str]) -> None:
    ...
    for sector in sectors:
        if sector not in valid_sectors:
            raise ValueError(
                f"The given sector {sector} is not valid. Must be one of {valid_sectors}."
            )
...
def validate_domains(domains: List[str]) -> None:
    ...
    for domain in domains:
        if domain not in valid_domains:
            raise ValueError(
                f"The domain {domain} is not valid, must be one of {valid_domains}."
            )
