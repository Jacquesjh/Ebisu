...
class Pipeline:
    ...
    def __init__(self, data: pd.DataFrame, filters: Dict[str, dict], optimizer: Dict[str, dict], constraints: Optional[dict] = None) -> None:
    ...
    def _apply_filters(self) -> None:
        for filter_name in self.filters:
            ...
            self.data = filter_function(df = self.data, parameters = self.filters.get(filter_name))


    def _constrained_weights(self, optimizer_function: Callable, parameters: dict) -> pd.DataFrame:
        from athena.src.services.constrainer.constrainer import constrained_weights

        weights = constrained_weights(raw_df = self.raw_data, filtered_df = self.data, constraints = self.constraints, optimizer_function = optimizer_function, parameters = parameters, date = self.date)

        return weights


    def _unconstrained_weights(self, optimizer_function: Callable, parameters: dict) -> pd.DataFrame:
    ...
    def get_weights(self) -> pd.DataFrame:
        ...
        opt_name, parameters = list(self.optimizer.keys())[0], list(self.optimizer.values())[0]
        ...
        weights = constraints[self.constraints is None](optimizer_function = optimizer_function, parameters = parameters)
