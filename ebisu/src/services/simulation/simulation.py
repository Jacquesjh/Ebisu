...
class Simulation:
    ...
    def run_universe(
        self, data: pd.DataFrame, start_date: datetime, num_possibilities: int
    ) -> pd.DataFrame:
        ...
        strategies_per_worker = list(
            utils.split_list(my_list=index_strategies, n_parts=self.num_workers)
        )

        future_dfs = []

        for _worker in range(self.num_workers):
            dfs = run_partial_universe.remote(
                indexes=strategies_per_worker[_worker],
    ...
    def get_many_strategies(self, data: pd.DataFrame, num_strategies: int) -> None:
        ...
        for i in range(num_strategies):
            strategy = get_strategy(
                ...
                max_num_filters=self.max_num_filters,
...
def backtest_strategy(
    data: pd.DataFrame, strategy: dict, start_date: datetime, rebalance_interval: str
) -> pd.DataFrame:
    backtest = Backtest(
        data=data,
