...
def get_random_strategy(
    total_tickers: int, num_filters: int, portfolio_size: int
) -> dict:
    pipeline = dict()

    pipeline["filters"] = get_random_filters(
...
def parameters_table_optimizer(optimizer_name: str) -> dict:
    table = {"hrp": hrp_parameter, "ewp": ewp_parameter}
...
def get_random_filters(
    total_tickers: int, portfolio_size: int, num_filters: int
) -> dict:
    filters = dict()
    ...
    for i in range(num_filters):
        ...
        filter_parameters = parameters_table_filters(
            filter_name=chosen_filter, num_tickers=num_tickers
        )
