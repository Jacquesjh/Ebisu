...
from athena.src.services.dashboard.configs import (RULES_AVAILABLE, SECTORS_AVAILABLE, DOMAINS_AVAILABLE, FILTERS_AVAILABLE, 
                                                   OPTIMIZERS_AVAILABLE, OPTIMIZERS_SECUNDARY_OPTIONS, FILTERS_SECUNDARY_OPTIONS,
                                                   RULES_SECUNDARY_OPTIONS, TICKERS_AVAILABLE)
...
app.layout = html.Div([
    ...
    html.Br(),
    html.Label("Do you want to add new a customized rule to your assets selection?", id = "add-rule"),
    ...
    html.Div(children = [
        ...
        html.Div(children = [
            ...
            dcc.Dropdown(FILTERS_AVAILABLE, id = {"type": "filter-dropdown", "index": f"filter-0"}),
...
@app.callback(Output("rules-layout", "children"),
              Input("add-rule-button", "n_clicks"), 
              State("rules-layout", "children"), prevent_initial_call = True)
def add_rule(click: int, children: dict) -> dict:
    component = html.Div(children = [

            html.Br(),
            html.Label("Select the rule you want in your asset selection.*", id = f"label-rule-{click}"),
            dcc.Dropdown(RULES_AVAILABLE, id = {"type": "rule-dropdown", "index": f"rule-{click}"}),
...
@app.callback(Output({"type": "rule-div", "index": MATCH}, "children"),
              Input({"type": "rule-dropdown", "index": MATCH}, "value"),
              State({"type": "rule-div", "index": MATCH}, "children"), prevent_initial_call = True)
def update_rules_options(value: str, children: dict) -> dict:
    if value is None:
        ...
        for option in rule_options:
            ...
            br     = html.Br()
            label  = html.Label(f"{name} you {negation} want.*", id = f"label-rule-{index}-{option}")
            option = dcc.Dropdown(drop_options, id = f"rule-options-{index}-{option}", multi = True)
...
@app.callback(Output("filters-layout", "children"),
              Input("add-filter-button", "n_clicks"), 
              State("filters-layout", "children"), prevent_initial_call = True)
def add_filter(click: int, children: dict) -> dict:
    component = html.Div(children = [

            html.Br(),
            html.Label("Select the filter you want.*", id = f"label-filter-{click}"),
            dcc.Dropdown(FILTERS_AVAILABLE, id = {"type": "filter-dropdown", "index": f"filter-{click}"}),
...
@app.callback(Output({"type": "filter-div", "index": MATCH}, "children"),
              Input({"type": "filter-dropdown", "index": MATCH}, "value"),
              State({"type": "filter-div", "index": MATCH}, "children"), prevent_initial_call = True)
def update_filters_options(value: str, children: dict) -> dict:
    if value is None:
        ...
        for option in filter_options:
            ...
            br     = html.Br()
            label  = html.Label(f"Select the {name} you want.*", id = f"label-filter-{index}-{option}")
            option = dcc.Input(type = "number", min = 1, step = 1, id = f"filter-options-{index}-{option}")
...
@app.callback(Output("optimizer-layout", "children"),
              [Input("select-optimizer", "value")],
              [State("optimizer-layout", "children")], prevent_initial_call = True)
def update_optimizer_options(value: str, children: dict) -> dict:
    if value is None:
        ...
        for option in optimizer_options:
            ...
            br     = html.Br()
            label  = html.Label(f"Select the {name} you want.", id = f"label-optimizer-{option}")
            option = dcc.Dropdown(optimizer_options[option], id = f"optimizer-options-{option}", value = optimizer_options[option][0])
...
@app.callback(Output("show-layout", "children"),
              Input("run-button", "n_clicks"),
              State("layout", "children"), prevent_initial_call = True)
def run(click: int, children: dict) -> dict:
    ...
    print("Backtesting...")
    backtest = Backtest(filters = filters, data_rules = rules, optimizer = optimizer, backtesting_years = period)
...
def get_rules(children: dict) -> dict:
    ...
    rules_layout = [child for child in children if child["props"].get("id", "") == "rules-layout"][0]
...
def get_filters(children: dict) -> dict:
    ...
    filters_layout = [child for child in children if child["props"].get("id", "") == "filters-layout"][0]
...
def get_optimizer(children: dict) -> dict:
    ...
    opt_layout   = [child for child in children if child["props"].get("id", "") == "optimizer-layout"][0]
...
def get_period(children: dict) -> int:
    period_comp = [child for child in children if child["props"].get("id", "") == "input-period"][0]
