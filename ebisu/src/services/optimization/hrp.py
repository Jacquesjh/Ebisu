...
class HRP(Optimizer):
    ...
    def __init__(self, historical_prices: pd.DataFrame, covariance_function: str, linkage_method: str) -> None:
    ...
    def _optimize(self) -> HRPOpt:
        ...
        if len(covariance.columns) == 1:                    # When there is only one asset
            hrp = pd.DataFrame([1.0], columns = covariance.columns, index = [self.historical_prices.index[-1]])
    ...
    def get_weights(self) -> pd.DataFrame:
        if type(self.hrp) == pd.DataFrame:
            ...
            weights = pd.DataFrame(np.array(list(weights.values())).reshape((1, num_tickers)), columns = weights.keys(), index = [date])
...
def hrp_optimizer(df: pd.DataFrame, parameters: dict) -> Callable:
    ...
    hrp = HRP(historical_prices = prices, covariance_function = covariance_function, linkage_method = linkage_method)
