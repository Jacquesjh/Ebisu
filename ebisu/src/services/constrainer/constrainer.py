...
class Constrainer:
    ...
    def __init__(
        self, raw_df: pd.DataFrame, filtered_df: pd.DataFrame, constraints: dict
    ) -> None:
    ...
    def _apply_constraints(self) -> dict:
        ...
        for constraint in self.constraints:
            ...
            dataframes = {
                **dataframes,
                **constraint_function(args=self.constraints[constraint]),
            }
    ...
    def _dynamic_percentage(self, low: float, high: float) -> float:
        ...
        return_track = (1 + returns.iloc[-lookback_return:]).cumprod().iloc[
            -1
        ].mean() - 1

        negative_returns = pd.DataFrame(
            np.where(returns.iloc[-20:] < 0, returns.iloc[-20:], 0)
        )
    ...
    def _invest_sections(
        self, args: Dict[float, List[str]]
    ) -> Dict[float, pd.DataFrame]:
        sections_dataframes = dict()

        for percentage, section in args.items():
            if type(percentage) == tuple:
                percentage = self._dynamic_percentage(
                    low=percentage[0], high=percentage[1]
                )
...
def constrained_weights(
    ...
    constrainer = Constrainer(
        raw_df=raw_df, filtered_df=filtered_df, constraints=constraints
    )
