...
class Data:
    ...
    def _include_tickers(self, args: List[str]) -> None:
        tickers = [
            ticker if ticker.endswith(".SA") else f"{ticker}.SA" for ticker in args
        ]
        self.tickers = self.tickers.union(tickers)
    ...
    def _exclude_sectors(self, args: List[str]) -> None:
        ...
        for domain in self.domains:
            ...
            for sector in args:
                ...
                if tickers is None:
                    raise ValueError(
                        f"The domain {domain} domain doesn't have {sector} sector. Only {list(domain_sectors.keys())}."
                    )
    ...
    def _download_data(self, yahoo: bool) -> None:
        if yahoo:
            ...
            data = dataingestion.get_data(
                needed_info=needed_info, start_date=start_date
            )
